...
In a system of Poor mans majoranas, braiding does not come from physically moving the Majorana modes or any other physical pieces of the setup, but rather by tuning the parameters of the system in a way that mimics the effect of an exchange. What this does, is couple the Majorana modes in a way that produces the same unitary transformation on the low-energy subspace as a physical braid would. This is sometimes called ``measurement-only'' or ``parameter-tuning'' braiding, and it relies on the same underlying topological principles as physical braiding, but it is implemented through a sequence of parameter changes rather than through spatial motion.\\
...
Lets start by introducing two quantum states $\ket{0}$ and $\ket{1}$, which we can simply represent as column vectors:
\begin{equation}
...
We are not going to introduce the Jordan-Wigner transformation because of its relevance to the Majorana systems, but rather because it provides a useful example of how to express quantum gates in terms of fermionic operators. The Jordan-Wigner transformation allows us to express the spin-1/2 Pauli operators in terms of fermionic creation and annihilation operators{\color{red}$https://futureofmatter.com/assets/fermions_and_jordan_wigner.pdf$}. For a chain of spin-1/2 particles, the transformation is given by:
\begin{align*}
\sigma_j^+ &= \frac{1}{2}(\sigma_j^x + i\sigma_j^y) \equiv f_j ^{†}, \\
\sigma_j^- &= \frac{1}{2}(\sigma_j^x - i\sigma_j^y) \equiv f_j, \\
\sigma_j^z &= 2f_j ^{†} f_j - 1,
\end{align*}
Leaving us with the correct same-site fermionic relations, but operators on different sites commute instead of anticommute. To fix this, we take a achain of fermions, and define a new set of fermionic operators:
\begin{align*}
