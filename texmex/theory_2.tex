...
{\color{red} Mention that it is physically meaningful to check both the \(X\) and \(Y\) local Majorana components.
...
\chi_y = i(c^\dagger-c).
\]
Choosing only \(\chi_y\) assumes that the physically relevant Majorana is aligned with that quadrature. But in an interacting/projected low-energy system, the effective Majorana can be rotated in this local Majorana plane,
