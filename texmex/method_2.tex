...
\end{equation}
{\color{red}  For non-interacting theories, this step shouldnt be necessary, but explain why}
...
% The optimization procedure returns a scalar loss and a set of optimized parameters. By picking the best configuration based on the lowest loss, and the desired number of quantum dots, as well as a chosen interaction strength $U$. We can run a post optimization diagnostic routine on the selected configuration. This allows us to analyze and verify the physical properties of the optimized Hamiltonian, and to check whether it has the expected Majorana-like signatures.
% This is a necessity, since the loss alone is one single scalar value, scoring 4 different contributions of varying depths. For example, the loss can be very low, and every contribution can be very low, but it still does not force the ground state to be perfectly degenerate. 
% After the process of optimizing parameters for a number of different configurations, we can select the best configuration for each pair of $(n,U)$, and run the post optimization diagnostic routine on it.  
% This is initiated by loading the stored optimization output $Φ_{\mathrm{opt}}(n,U)$, and rebuilding the physical Hamiltonian $H_{\mathrm{PMM}}(\theta_{\mathrm{opt}})$ from the optimized parameters. The reconstructed Hamiltonian is then analyzed through a series of diagnostics presented in this section.   


 
% The optimization procedure returns a scalar loss and a set of optimized parameters, but the loss alone is not suﬀicient to decide whether the result has a useful Majorana
% interpretation.After each optimization run, the best parameter set is therefore reconstructed and analyzed with a separate diagnostic workflow. The purpose of this step is to translate the numerical minimum into physical quantities that can be compared across system sizes, interaction strengths, and later braiding calculations.

% \subsection{Reconstruction of optimized Hamiltonians}
...
% \begin{equation}
% H_{\mathrm{PMM}}(\theta_{\mathrm{opt}})
% \end{equation}
...
\end{equation}
A small value of $\delta_0$ indicates that the lowest even and odd states are approximately degenerate. However, this degeneracy is only useful if the pair is also separated from excited states. For this reason, the low-energy gap is defined as
\begin{equation}
\Delta_{\mathrm{gap}}
=
...
\end{equation}
after the common ground-state energy shift. This quantity measures the distance from the lowest parity pair to the nearest excited state in either parity sector. The combination of small $\delta_0$ and large $\Delta_{\mathrm{gap}}$ is the main spectral criterion used to identify promising optimized chains.
...
\end{equation}
For two-dot systems there are no interior sites, so the bulk contribution is set to zero. A good Majorana-like configuration should have $\overline{|M_{\mathrm{edge}}|}$ close to one and $\max |M_{\mathrm{bulk}}|$ close to zero.
...
Although Majorana physics is often discussed in terms of the lowest parity pair, the optimization used here was deliberately applied to the full parity-resolved spectrum of the finite many-body Hamiltonian. This choice was motivated by the later sector-resolved braiding analysis, where each identified energy sector of the full three-subsystem Hamiltonian is projected and tested independently. The goal was therefore not only to obtain a clean ground-state Majorana signature, but also to find parameter sets whose parity pairing, excitation gaps, charge structure, and Majorana-polarization properties remain well behaved beyond the lowest states.

The post-optimization diagnostics therefore serve two purposes. First, they identify whether the lowest parity pair has the expected Majorana-like structure. Second, they indicate whether the optimized configuration remains suitable when higher energy sectors are tested in the braiding calculations. Configurations that satisfy both requirements are selected as candidates for the effective Majorana construction described next.
...
For an operator $\gamma_\alpha$ acting on subsystem $\alpha$, the embedding is schematically
\begin{equation}
...
\subsection{Transported bands used in the calculations}
