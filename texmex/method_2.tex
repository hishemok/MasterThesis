...
These splittings are weighted more strongly at low energy, because the ground-state degeneracy is the most important Majorana signature. If $N_p$ parity pairs are used, the implementation defines a decreasing weight array
\begin{equation}
w_{\ell}
=
\left[
...
The Majorana-polarization penalty compares the site-resolved Majorana-polarization profile with the expected edge-localized pattern. Numerically, the polarization for parity pair $k$ and site $j$ is evaluated from the parity-changing matrix elements of the two local Majorana components $\gamma_{j,1}=c_j^\dagger+c_j$ and $\gamma_{j,2}=i(c_j^\dagger-c_j)$,
\begin{equation}
M_{k,j}
=
\left(
...
\begin{equation}
M^{\mathrm{tar}}_{k,1}=1,
\qquad
M^{\mathrm{tar}}_{k,n}=-1,
\qquad
M^{\mathrm{tar}}_{k,j}=0
\quad
(1<j<n).
\end{equation}
...
M_{k,j}
-
\sigma M^{\mathrm{tar}}_{k,j}
\right)^2
\right|.
\end{equation}

\subsection{Total Normalized Loss}
The full loss is a weighted sum of the spectral and local contributions. The implemented total loss is
\begin{equation}
\mathcal{L}
=
\frac{
...
\end{equation}
where $\overline{|E|}$ is the mean absolute energy scale of the spectrum and $\eta$ is a small numerical offset.
...
\ket{o_j}\bra{e_j}
\right).
\label{eq:method_energy_level_majorana_components}
\end{equation}
...
The projected junction operators are also decomposed in the energy-level
Majorana-bilinear basis. This identifies which low-energy Majorana channel is
selected by the physical endpoint coupling. In the low-energy benchmark, this
decomposition is used to choose which Majorana component from subsystems $B$
and $C$ enters the local-operator model below.
...
Hamiltonian. For an endpoint dot in subsystem $S=B,C$, the two microscopic
components are
\begin{equation}
...
\end{equation}
After projection and normalization, the component selected by the junction
analysis defines the local Majorana operator used in the braid,
\begin{equation}
\gamma_{S,\mathrm{loc}}
=
\mathcal{N}_S V^\dagger \chi_{S,\pm} V .
\end{equation}

The corresponding local-operator braid Hamiltonian is
\begin{equation}
H_{\mathrm{loc}}(t)
=
\lambda_1(t)\, i\gamma_0\gamma_1
+
\lambda_2(t)\, i\gamma_0\gamma_{B,\mathrm{loc}}
+
\lambda_3(t)\, i\gamma_0\gamma_{C,\mathrm{loc}} .
...
In the projected calculations, the same operator-action check is evaluated with the target Majoranas appropriate to the construction being tested. For the energy-level construction, $γ_2$ and $γ_3$ are the Majorana operators constructed from even--odd parity pairs, as described in Sec.~\ref{sec:method-effective-majorana-construction}. For the projected-local and block-matched constructions, the corresponding target operators are instead built from the projected local endpoint Majorana components.
...
\|U_{eo}\|.
\end{equation}
A small value indicates that the braid approximately preserves fermion parity inside the transported manifold, which always has to be the case and serves as a check on the numerics.
