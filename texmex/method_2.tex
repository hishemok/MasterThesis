...
Degeneracy alone is not sufficient: the parity-paired states must also be separated from nearby levels. The second loss contribution penalizes small gaps within the parity-resolved spectrum. For adjacent levels, the relevant gap is taken as the smaller of the even-sector and odd-sector spacings,
\begin{equation}
\Delta_k
=
\min\left(
...
\end{equation}
where \(\eta_{\mathrm{Adam}}
\) is the learning rate, and  \(\hat{m}_k\) and \(\hat{v}_k\) are the bias-corrected exponential moving
averages of the gradient and squared gradient, respectively \cite{kingma2014adam}.
...
Using the parity classification introduced in Sec.~\ref{sec:theory-parity-resolved-spectrum}, the reconstructed Hamiltonian is diagonalized and separated into ordered even- and odd-parity spectrum using the total fermion parity operator
\begin{equation}
P_{\mathrm{tot}}=\prod_{i=1}^{n}(1-2n_i).
\end{equation}
...
\end{equation}
where \(d_T\) is the dimension of the transported subspace. A perfect match up to a global phase gives \(\mathcal{O}=1\), so the plotted quantity \(D_{\mathcal{O}}\) vanishes for an ideal braid and increases as the transported unitary deviates from the target.
