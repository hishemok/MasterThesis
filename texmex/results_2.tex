...
\end{figure}
\Cref{fig:braiding_junctionprotocol} shows the resulting braid after replacing the energy-level Majorana bilinears with the projected microscopic junction operators. The spectral evolution remains almost identical to the energy-level benchmark, with a well-isolated low-energy manifold throughout the cycle. The maximum splitting inside this manifold increases only to $\max(E_3-E_0)=7.18\times 10^{-7}$, while the minimum gap remains large, $\min(E_4-E_3)=1.413$.
...
\begin{table}[htbp]
  ...
  \input{generated/braiding_checks_comparison.tex}
