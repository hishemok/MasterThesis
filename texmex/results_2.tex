...
When exploring the different energy sectors, we can create projections of different dimensions. For example, $P_0 = P_{\mathrm{GS}}$, the ground-state projection described by the group size in \cref{fig:full_hamil_elevels}, has dimension $8 \times 512$. The factor $8$ comes from the number of states in the ground-state sector, while $512$ is the total number of states in the full system. If we look at the first excited sector, we can create a projection $P_1$ that has dimension $24 \times 512$, and so on. An interesting observation occurred when we projected the system down to any of its energy sectors. After the braid couplings were activated, the projected spectrum split into a lower and an upper band of equal dimension, separated by a gap \cref{fig:level_split}. This split provides a well-defined lower band for Kato transport, but does not by itself establish that the resulting unitary performs a successful Majorana exchange. The target-overlap results below provide that separate test.
...
\begin{figure}[htbp]
  ...
  \caption{Self-target braid errors for the block-matched local-operator model. The same three deviation regimes as in \cref{fig:projected_operator_interaction_regimes} remain visible: a) $U=1.0$ and $U=2.0$, b) $U=0.5$, and c) $U=0.0$ and $U=0.1$.
 }
