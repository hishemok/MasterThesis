...
During the scans, each energy sector was found to decompose into blocks whose dimensions are multiples of the eight-dimensional three-subsystem ground sector. For example, a $24$-dimensional sector contains three independent $8\times8$ blocks, as illustrated in \cref{fig:24x24box,fig:24x24maj}. A single overall sign cannot generally align the projected local operator with the energy-level Majorana across all of these blocks at once.

To account for this structure, we use a block-matching procedure whose full construction will be described in {\color{red}the appendix [ADD REFERENCE]}. The procedure first identifies the independent blocks using subsystem energy-level labels. Within each block, the projected local operator is fitted independently to a continuous linear combination of the two available energy-level Majorana components. The independently fitted blocks are then reassembled into one operator on the full sector. The diagrams below provide a short visual summary of this decomposition.
