...
In this section, we will review the parameters found by the basin-hopping optimization procedure, and how these parameters depend on the system size $n$ and the fixed interaction strength $U$. As mentioned in the methods section, the optimization is performed for chain lengths $n=2,3,$ and $4$, and for interaction strengths $U=0,0.1,0.5,1.0,2.0,$ and $5.0$, with a fixed hopping amplitude $t_i=1$ for all $i$. The purpose of the fixed hopping amplitude was to make sure that the hopping amplitude $t_i$ and the pairing amplitude $Δ_i$ do not make the implication of the interaction strength vanish, by scaling the other parameters up. The fixed hopping amplitude also makes it easier to compare the optimized pairing amplitudes $\Delta_i$ across different interaction strengths and system sizes, since they are all measured relative to the same fixed scale. The main purpose of this section is therefore to summarize the best parameter sets before applying the more detailed diagnostics in the following section, and to identify any trends in the optimized parameters as the interaction strength and system size are varied.  
...
\begin{figure}
  ...
  \caption{Parity-resolved spectra after optimization for the two-dot system at $U=0$ and $U=0.1$. The optimized parameters are shown schematically below each spectrum.}
...
\begin{figure}
  ...
  \caption{Parity-resolved spectrum after optimization for the two-dot system at $U=2.0$. The optimized parameters are shown schematically below the spectrum.}
