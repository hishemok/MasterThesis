...
The raw errors therefore do change as the projection space is enlarged. For both $U=0$ and $U=0.1$, the raw odd/even target-gate errors grow from approximately $1.34 \times 10^{-6}$ in the ground-state block to approximately $1.07 \times 10^{-5}$ in the full projected space. The normalized errors remain essentially constant because the ideal projected-Majorana braid acts as the same exchange operation repeated across the included parity blocks, so the Frobenius-norm error scales approximately as the square root of the block dimension.

The same pattern appears in the single-exchange Majorana error. The raw error grows with the size of the transported manifold, while the dimension-normalized value remains approximately $1.86 \times 10^{-3}$. This means that the constant normalized errors are not a sign that the table failed to update; they indicate that the error per included block is roughly unchanged in this ideal projected-Majorana calculation.

This also means that Table \ref{tab:extended_projection_braiding} should be interpreted as an ideal control calculation, not as evidence that the Coulomb interaction is irrelevant. In that table the effective braid is constructed from normalized projected Majorana operators, so once the algebra has been enforced the remaining error is mostly a numerical and dimensional normalization effect. To test the interaction dependence more directly, I also repeat the calculation using the physical projected junction operators, where the microscopic coupling is projected into the enlarged manifold instead of replacing it by the ideal Majorana bilinear.
