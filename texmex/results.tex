...
The physical junction is taken to be the projected local minus operator, $i(c^\dagger-c)$, on the inner B and C sites. This is the local channel that overlaps with the participating Majoranas in the ground-state junction decomposition. I keep the ideal projected-Majorana braid as a control calculation, but the physically relevant diagnostics are the physical target-gate error, the physical-versus-ideal braid mismatch, the transported-band exchange error on the ideal Majorana channels, and the projected physical-Majorana algebra error.

\begin{table}[p]
  \centering
  \scriptsize
  \setlength{\tabcolsep}{4pt}
  \resizebox{\textwidth}{!}{\input{generated/retry_projection_scan_summary_retry2_minus_only_extended.tex}}
  \caption{Compact summary of the corrected higher-energy braid scan. All errors are normalized. The transported dimension $\dim T$ is selected from the spectral gap of the time-dependent projected Hamiltonian. ``Ideal gate'' is the internally consistent projected-Majorana control braid. ``Phys.\ gate'' uses the projected microscopic minus-channel junctions. ``Phys.\ vs.\ ideal'' compares the physical and ideal braid unitaries in the ideal transported band. ``Transp.\ exch.'' is the maximum transported-band exchange error of the physical braid on the ideal Majorana channels, and ``Phys.\ Maj.'' is the maximum normalized physical-Majorana square error.}
  \label{tab:retry_projection_scan_summary}
\end{table}

Table \ref{tab:retry_projection_scan_summary} gives the overall trend. The ideal control braid remains accurate for every interaction strength because it is generated from, and compared against, the same projected Majorana algebra. The physically meaningful columns show the expected interaction hierarchy instead: the noninteracting case remains good across the scan, the weakly interacting case is slightly worse but still close through the moderate projections, and the strongly interacting case is clearly separated by both the physical-versus-ideal mismatch and the transported exchange error.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_interaction_comparison_retry2_minus_only.pdf}
  \caption{Interaction comparison for the corrected minus-channel projection scan, restricted to $\dim P \le 80$. The left panel shows the normalized target-gate error, while the right panel shows the transported-band exchange error on the ideal Majorana channels. The dashed ideal curves are control calculations and remain near the numerical integration floor. The physical curves show the expected separation: $U=0$ stays small, $U=0.1$ is slightly worse, and $U=2$ is strongly degraded once excited sectors enter.}
  \label{fig:retry_projection_interaction_comparison_dim80}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{figs/retry_projection_direct_match_diagnostic_retry2_minus_only.pdf}
  \caption{Direct physical-versus-ideal braid mismatch for $\dim P \le 80$. This is the cleanest diagnostic of whether the microscopic minus-channel junction implements the same braid as the ideal projected-Majorana Hamiltonian. The weakly interacting case remains close to the noninteracting result, while the $U=2$ mismatch is already large at the first excited projections.}
  \label{fig:retry_projection_direct_match_dim80}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_channel_breakdown_retry2_minus_only.pdf}
  \caption{Transported-band channel-resolved exchange errors for $\dim P \le 80$. Unlike the earlier full-projection operator diagnostic, this plot evaluates $V^\dagger(U^\dagger\gamma_iU-\gamma_j)V$ inside the transported band $V$. The exchanged channels therefore agree with the target-gate diagnostics: $U=0$ remains close to the ideal exchange, $U=0.1$ gives an intermediate deviation, and $U=2$ fails primarily in the intended exchange channels.}
  \label{fig:retry_projection_channel_breakdown_dim80}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_interaction_comparison_retry2_minus_only_extended.pdf}
  \caption{The same interaction comparison extended to $\dim P = 256$ and $512$. The full-range plot documents the high-dimensional endpoints used in Table \ref{tab:retry_projection_scan_summary}. The noninteracting target-gate error remains small even at the largest projection sizes, but the physical-versus-ideal diagnostics in the companion plots reveal that high-dimensional projected spaces can still expose basis mismatch in the microscopic junction description.}
  \label{fig:retry_projection_interaction_comparison_dim512}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_channel_breakdown_retry2_minus_only_extended.pdf}
  \caption{Transported-band channel-resolved exchange errors for the full $\dim P \le 512$ scan. The high-dimensional endpoints preserve the same qualitative hierarchy: the ideal control remains accurate, the weakly interacting case is intermediate, and the strongly interacting case has order-one exchange-channel mismatch. The extended scan is mainly a robustness check; the $\dim P \le 80$ window remains easier to interpret visually.}
  \label{fig:retry_projection_channel_breakdown_dim512}
\end{figure}

Figures \ref{fig:retry_projection_interaction_comparison_dim80}--\ref{fig:retry_projection_channel_breakdown_dim80} are the most useful plots for interpreting the moderate-projection behaviour. They show that the apparent failures in the earlier full-space exchange diagnostic were caused by comparing operators outside the transported band. After restricting the exchange test to the same band used in the target-gate comparison, the channel breakdown and target-gate metrics tell the same story.

Figures \ref{fig:retry_projection_interaction_comparison_dim512} and \ref{fig:retry_projection_channel_breakdown_dim512} extend the diagnostics to $\dim P=256$ and $512$. These endpoints are useful for documenting the full numerical scan, but the larger projected spaces also make the distinction between the ideal projected-Majorana control and the microscopic physical junction more visible. This is why the direct physical-versus-ideal mismatch is retained as a separate diagnostic rather than relying only on the physical target-gate error.
