...
\begin{table}[p]
  ...
  \scriptsize
  \setlength{\tabcolsep}{4pt}
  \resizebox{\textwidth}{!}{\input{generated/retry_projection_scan_summary_new_bc_crosslevel_dim512.tex}}
  \caption{Compact summary of the updated higher-energy projection scan using the new cross-level fitted B and C Majoranas. The scan now includes $\dim P = 8, 32, 56, 80, 256,$ and $512$. ``Ideal'' columns refer to the braid generated by the fitted projected Majoranas, while ``Phys.'' columns use the projected microscopic junction operators directly. In the updated construction these two descriptions agree to numerical precision, so the ``vs.\ ideal'' columns remain near machine precision throughout and mainly serve as a consistency check.}
  \label{tab:retry_projection_scan_summary}
\end{table}

Table \ref{tab:retry_projection_scan_summary} already shows the main trend in the updated construction. With the new cross-level fit for the B and C Majoranas, the fitted ``ideal'' operators and the directly projected microscopic operators now lie on top of each other to numerical precision, so the old ideal-versus-physical basis mismatch is no longer the main story. The relevant dependence is instead on interaction strength and projection size. For $\dim P=8$ all three interaction strengths remain essentially identical, the weakly interacting cases $U=0$ and $U=0.1$ stay comparatively small through $\dim P=80$, while the $U=2$ errors grow rapidly once excited sectors are included. The scan extended to $\dim P=256$ and $512$ is included below, but the $0$--$80$ window remains the clearest place to see the intermediate-manifold trend before the full-space endpoint compresses the vertical scale.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_interaction_comparison_new_bc_crosslevel_dim80.pdf}
  \caption{Interaction comparison for the updated cross-level Majorana construction, restricted to $\dim P \le 80$. This zoomed view shows the regime where the separation by interaction strength is easiest to read. The $U=0$ and $U=0.1$ curves remain relatively small through $\dim P=80$, while $U=2$ rises sharply once the first excited sectors are included. The ideal and physical curves overlap to plotting precision in the updated construction, so the visible trend is the genuine interaction dependence rather than a basis-convention artifact.}
  \label{fig:retry_projection_interaction_comparison_dim80}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_channel_breakdown_new_bc_crosslevel_dim80.pdf}
  \caption{Channel-resolved single-exchange errors for the same $\dim P \le 80$ window. The dominant interaction dependence still sits in the exchanged channels $\gamma_2 \rightarrow -\gamma_3$ and $\gamma_3 \rightarrow \gamma_2$, while the spectator channels remain much smaller. This is the clearest figure for the pre-$256$ behaviour and is therefore the most useful one for discussing the gradual loss of exchange quality with increasing interaction strength.}
  \label{fig:retry_projection_channel_breakdown_dim80}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_interaction_comparison_new_bc_crosslevel_dim512.pdf}
  \caption{The same interaction comparison extended to $\dim P = 256$ and $512$. This figure documents the full scan range used in Table \ref{tab:retry_projection_scan_summary}. Because the additional high-dimension points broaden the vertical range, the lower-dimensional behaviour is harder to read here than in Figure \ref{fig:retry_projection_interaction_comparison_dim80}; this is precisely why the $0$--$80$ zoom is kept alongside the full scan.}
  \label{fig:retry_projection_interaction_comparison_dim512}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/retry_projection_channel_breakdown_new_bc_crosslevel_dim512.pdf}
  \caption{Channel-resolved single-exchange errors for the full $\dim P \le 512$ scan. The dominant contribution again comes from the exchanged channels, while the spectator channels remain much smaller. Compared with Figure \ref{fig:retry_projection_channel_breakdown_dim80}, the extended scan is less suited for reading off the low-lying trend but is useful for documenting how the same channel hierarchy persists once the $256$ and $512$ points are included.}
  \label{fig:retry_projection_channel_breakdown_dim512}
\end{figure}

Figure \ref{fig:retry_projection_interaction_comparison_dim80} gives the clearest summary of the updated scan in the physically most readable range. In the ground-state manifold the three interaction strengths are essentially indistinguishable, but after the first excited blocks are included the errors separate clearly by $U$. The weakly interacting cases $U=0$ and $U=0.1$ stay relatively small through $\dim P=80$, whereas the $U=2$ curves rise to order $10^{-1}$ and then order unity. The updated construction therefore preserves the qualitative conclusion of the earlier scan while removing the spurious ideal-versus-physical basis split.

Figure \ref{fig:retry_projection_channel_breakdown_dim80} shows that the degradation remains concentrated in the exchanged channels. The channels $\gamma_2 \rightarrow -\gamma_3$ and $\gamma_3 \rightarrow \gamma_2$ carry the dominant interaction dependence, while $\gamma_1 \rightarrow \gamma_1$ and especially $\gamma_0 \rightarrow \gamma_0$ remain far more stable. The enlarged-manifold braid therefore still fails in a structured way: the intended exchange deteriorates first, rather than every projected operator becoming equally corrupted.

Figures \ref{fig:retry_projection_interaction_comparison_dim512} and \ref{fig:retry_projection_channel_breakdown_dim512} extend the same diagnostics to $\dim P=256$ and $512$. I keep these full-range plots in the thesis to document the large-projection behaviour explicitly, but the $0$--$80$ versions remain the more informative figures for discussing the interaction-driven trend before the highest-dimensional endpoints compress the scale.
