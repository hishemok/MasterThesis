...
def plot_projected_hamiltonian_levels(projected_majoranas, bases, full_parity, idx, t=0.0):
...
def evolve_system(n_steps, projected_majoranas, idx, bases, h_full):
    TA, TB, TC = get_braiding_terms(projected_majoranas, idx)
...
def build_sector_data(U_value):
    ...
    projected_majoranas = project_majoranas(
        bases,
        gamma_A1,
        gamma_A2,
        gamma_B1,
    ...
    return h_full, bases, projected_majoranas


def write_results(U_value, results):
    results_path = cwd / f"braiding_results_step_projected_braiding_U={U_value}.txt"
    with open(results_path, "w") as file:
        file.write("Group\tOverlap\n")
        for idx, overlap in results.items():
            file.write(f"{idx}\t{overlap:.10f}\n")


def run_scan(U_values=(0.0, 0.1, 0.5, 1.0, 2.0), n_steps=300):
    for U_value in U_values:
        print(f"\nProcessing U={U_value}")
        h_full, bases, projected_majoranas = build_sector_data(U_value)

        results = {}
        for idx in range(len(projected_majoranas)):
            U_kato, initial_basis = evolve_system(
                n_steps,
                projected_majoranas,
                idx,
                bases,
            ...
            U_target = get_target_unitary(projected_majoranas, idx)
            ...
            print(f"Group {idx:02d}: overlap={results[idx]:.10f}")

        write_results(U_value, results)
